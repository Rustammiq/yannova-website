...
import { YannovaApiClient } from './api-client.js';
import { ChatMessage } from './types.js';

class YannovaChatServer {
  private server: Server;
  private apiClient: YannovaApiClient;

  constructor() {
    this.apiClient = new YannovaApiClient(process.env.YANNOVA_API_URL);
      ...
      try {
        switch (name) {
          ...
          case 'generate_project_ideas':
            return await this.handleGenerateProjectIdeas(args as any);
  ...
  private async handleChat(args: { message: string; history?: ChatMessage[] }) {
  ...
  private async handleGetServices() {
    const services = await this.apiClient.getServices();
  ...
  private async handleGenerateProjectIdeas(args: { type?: string; budget?: string; location?: string }) {
    const { type, budget, location } = args;

    const prompt = `Genereer bouwproject ideeën voor Yannova Bouw.${
      type ? ` Focus op ${type}.` : ''
    }${
      budget ? ` Budget: ${budget}.` : ''
    }${
      location ? ` Locatie: ${location}.` : ''
    } Geef 3 concrete projectvoorstellen met kostenraming en tijdsduur.`;
