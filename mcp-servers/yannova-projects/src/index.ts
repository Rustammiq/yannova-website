...
import { YannovaProjectsApiClient } from './api-client.js';
import { CreateProjectRequest, UpdateProjectRequest, ProjectFilter } from './types.js';

class YannovaProjectsServer {
  private server: Server;
  private apiClient: YannovaProjectsApiClient;

  constructor() {
    this.apiClient = new YannovaProjectsApiClient(process.env.YANNOVA_API_URL);
  ...
  private setupToolHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          case 'get_projects':
            return await this.handleGetProjects(args as any);

          case 'get_project':
            return await this.handleGetProject(args as any);

          case 'create_project':
            return await this.handleCreateProject(args as any);

          case 'update_project':
            return await this.handleUpdateProject(args as any);

          case 'delete_project':
            return await this.handleDeleteProject(args as any);

          case 'filter_projects':
            return await this.handleFilterProjects(args as any);
          ...
          case 'estimate_project_cost':
            return await this.handleEstimateProjectCost(args as any);
  ...
  private async handleGetProjects(args: { filter?: ProjectFilter }) {
    let projects;
  ...
  private async handleGetProject(args: { id: string }) {
    const { id } = args;
  ...
  private async handleCreateProject(args: CreateProjectRequest) {
  ...
  private async handleUpdateProject(args: UpdateProjectRequest) {
  ...
  private async handleDeleteProject(args: { id: string }) {
    const { id } = args;
  ...
  private async handleFilterProjects(args: { status?: string; type?: string; location?: string }) {
    const filter: ProjectFilter = {};

    if (args.status) filter.status = args.status as any;
  ...
  private async handleGetProjectsSummary() {
    const projects = await this.apiClient.getProjects();
    const stats = await this.apiClient.getProjectStats();
  ...
  private async handleEstimateProjectCost(args: { type: string; size?: string; location?: string; features?: string[] }) {
    ...
    const projectType = Object.keys(baseCosts).find(key =>
    ...
    const estimatedCost = Math.round(
      baseCost.avg * sizeMultiplier * featureMultiplier * locationMultiplier
    );
